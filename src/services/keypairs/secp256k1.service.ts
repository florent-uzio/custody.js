  ...
  sign(privateKeyPem: string, message: string): string {
    ...
    if (!isString(privateKeyPem)) {
      throw new CustodyError({
        reason: "Invalid private key: Must be PEM-encoded secp256k1 private key",
      })
    }
    ...
    try {
      ...
      if (key.asymmetricKeyType !== "ec") {
        throw new CustodyError({
          reason: "Invalid private key: Must be a secp256k1 private key",
        })
      }
      // Verify it's secp256k1 curve (if details are available)
      const keyDetails = key.asymmetricKeyDetails
      if (keyDetails && keyDetails.namedCurve !== "secp256k1") {
        throw new CustodyError({
          reason: "Invalid private key: Must be a secp256k1 private key",
        })
      }
      ...
      throw new CustodyError({
        reason: "Invalid private key: Failed to parse PEM-encoded secp256k1 private key",
      })
    }
