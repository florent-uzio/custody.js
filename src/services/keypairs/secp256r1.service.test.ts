...
describe("Secp256r1Service", () => {
  ...
  describe("sign", () => {
    ...
    it("should produce different signatures for the same message (ECDSA is probabilistic)", () => {
      // ECDSA/secp256r1 uses randomness (nonce) during signing, so the same message
    ...
    it("should throw CustodyError when private key is not a string", () => {
      ...
      expect(() => service.sign(null as any, "message")).toThrow(
        "Invalid private key: Must be PEM-encoded secp256r1 private key",
      )
    ...
    it("should throw CustodyError when private key has PEM header but is invalid", () => {
      ...
      expect(() => service.sign(invalidKey, "message")).toThrow(CustodyError)
      expect(() => service.sign(invalidKey, "message")).toThrow(
        "Invalid private key: Failed to parse PEM-encoded secp256r1 private key",
      )
    })

    it("should throw CustodyError when private key is wrong curve (secp256k1)", () => {
      // Generate a valid secp256k1 key (different curve) - covers lines 82-86
      const secp256k1Service = new Secp256k1Service()
      const wrongCurveKey = secp256k1Service.generate().privateKey

      expect(() => service.sign(wrongCurveKey, "message")).toThrow(CustodyError)
      expect(() => service.sign(wrongCurveKey, "message")).toThrow(
        "Invalid private key: Must be a secp256r1 private key",
      )
    })

    it("should throw CustodyError when private key is wrong curve (Ed25519)", () => {
    ...
    it("should re-throw CustodyError when validation throws CustodyError (covers lines 88-90)", () => {
      // This test ensures that when a CustodyError is thrown inside the try block,
      // it gets re-thrown correctly (lines 88-90)
      // The secp256k1 test above should already cover this, but let's be explicit
      const secp256k1Service = new Secp256k1Service()
      const wrongCurveKey = secp256k1Service.generate().privateKey

      try {
        service.sign(wrongCurveKey, "message")
        expect.fail("Should have thrown CustodyError")
      } catch (error) {
        // Verify it's a CustodyError (not wrapped in another error) - covers lines 88-90
