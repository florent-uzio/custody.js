...
const createMptWithFiveFlags = async () => {
  try {
    ...
    const { payloadId } = await custody.xrpl.proposeIntent(
      {
    ...
    // The transaction order ID: generated by the SDK unless you pass
    // `options.payloadId`. Use it to filter the transactions, or to resolve the
    // issuance this create minted with `custody.xrpl.getMptIssuanceIdAndWait`.
