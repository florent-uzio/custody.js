...
const createMpt = async () => {
  try {
    ...
    // `options.payloadId`. It is the handle to everything this order produced
    ...
    // The issuance ID is minted by the ledger, so it only exists once the intent
    // executed. Custody fills the transaction's ledger data in shortly *after*
    // that, which is why this polls rather than reading once
    const issuanceId = await custody.xrpl.getMptIssuanceIdAndWait({ domainId, payloadId })

    // Use this to authorize holders, send the token, or clawback
